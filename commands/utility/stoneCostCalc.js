...
async function generateUWChartImage(uwKey, chart, statFilterIdx = null, statsEntered = null, targetLevels = null) {
    ...
    if (typeof statFilterIdx === 'number' && !isNaN(statFilterIdx) && statsEntered && targetLevels) {
        ...
        // Only include rows for the selected stat's entered+1-to-target range (exclude starting level)
        ...
        if (statsToShow.length === 1) {
            ...
            if (statLevelsCounts[splitStatIdx] > 30) {
                ...
                for (let rowIdx = 0; rowIdx < allRows.length; rowIdx++) {
                    ...
                    // For single stat, not split, blank the Cost column for the last data row
                    let costCell = '';
                    // Only show cost for each row except the last data row
                    if (rowIdx !== allRows.length - 1) {
                        costCell = r.cost;
                    } else {
                        costCell = '';
                    }
                    const cells = [r.level, r.stat, costCell, r.cumulative];
                    for (let i = 0; i < columns.length; i++) {
                        ctx.strokeStyle = '#333';
                        ctx.lineWidth = 1;
                        ctx.strokeRect(x, y, colWidths[i], rowHeight);
    ...
    function buildSummaryRow(columns, statsToShow, statMaxValues, statTotals, splitStatIdx, splitStatRepeat, showCumulative) {
        ...
        let statMaxIdx = 0;
        let pairIdx = 0;
        // Special handling for single-stat, non-split: blank the Cost column in summary row
        const isSingleStatNonSplit = (splitStatIdx === null && statsToShow.length === 1 && columns.length === 4);
        for (let i = 0; i < columns.length; i++) {
            const col = columns[i];
            if (i === 0) {
            ...
            if (isSingleStatNonSplit) {
                // columns: [Level, Stat, Cost, Cumulative]
                if (col === 'Cost') {
                    summaryCells.push(''); // blank cost in summary row
                    continue;
                }
                if (col === 'Cumulative') {
    ...
    if (splitStatIdx !== null) {
        ...
        if (statsToShow.length === 1) {
            ...
            for (let rowIdx = 0; rowIdx < filteredRows.length; rowIdx++) {
                ...
                allRows.push({
                    level: row.level,
                    stat: statVal !== undefined ? statVal : '',
                    cost: '', // cost column blank
                    cumulative: (costVal && costVal !== 0) ? num(cumulative) : (cumulative ? num(cumulative) : '')
                });
            }
            if (statLevelsCounts[splitStatIdx] > 30) {
                // Split into two columns
                ...
                for (let rowIdx = 0; rowIdx < allRows.length; rowIdx++) {
                    const r = allRows[rowIdx];
                    const cells = [r.level, r.stat, '', r.cumulative]; // cost column blank
                    tableRowsForConsole.push(cells);
                }
            }
        } else {
        ...
        // Single stat selected (not split): show cost only in cumulative column
        if (statsToShow.length === 1) {
            let cumulative = 0;
            for (let rowIdx = 0; rowIdx < filteredRows.length; rowIdx++) {
                const row = filteredRows[rowIdx];
                ...
                const costVal = row.costs[0];
                cumulative += costVal || 0;
                // Only show cost for each row except the summary/total row
                // For the last data row, leave cost column blank to avoid duplicate total
                let costCell = '';
                if (rowIdx !== filteredRows.length - 1) {
                    costCell = (costVal && costVal !== 0) ? num(costVal) : '';
                }
                const cells = [row.level, statVal, '', num(cumulative)];
                tableRowsForConsole.push(cells);
            }
        } else {
            for (let rowIdx = 0; rowIdx < filteredRows.length; rowIdx++) {
...
function buildEmbed(state) {
    ...
    if (state.chart && uw.stats.length > 0) {
        ...
        if (statFilterIdx !== null) {
            // Only show the filtered stat's totals
        ...
        embed.addFields(
            { name: 'Stones Spent', value: num(spent), inline: true },
            { name: 'Remaining', value: num(remaining), inline: true },
            { name: 'Total Stones', value: num(total), inline: true }
        );
                        ...
                        modal.addComponents(
                            new ActionRowBuilder().addComponents(
                                new TextInputBuilder()
                                    ...
                                    .setLabel(`${stat.name} (${minLevel} - ${maxLevel})`)
